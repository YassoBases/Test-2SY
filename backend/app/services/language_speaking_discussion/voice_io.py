...
async def transcribe_discussion_audio(
    ...
    *,
    ...
    # Prefer local Whisper first — OpenAI STT often hits 429 and can burn the
    # full SPEAKING_TRANSCRIPTION_TIMEOUT_SECONDS before failing.
    ...
    for provider_name in providers:
        try:
            ...
            last_exc = exc
            logger.warning(
                "Discussion STT provider=%s failed; trying fallback if available",
                provider_name,
                exc_info=True,
            )
    ...
    if result is None:
        logger.warning(
            "Discussion dedicated STT failed after fallbacks; trying shared language STT",
            exc_info=last_exc,
        )
        try:
