...
def _peek_next_unlockable(
    *,
    ...
    simulated_completed = set(completed_ids)
    if learning_current:
...
def compute_progression_snapshot(
    *,
    ...
    unlocked = _expand_unlocks(
        eligible_ids=eligible_ids,
        by_id=by_id,
        seed_unlocked=frozenset(student.unlocked_ids),
        completed_ids=frozenset(student.completed_ids),
        candidate_ids=candidate_set,
    )
    ...
    learning_queue = [gid for gid in unlocked_sorted if gid not in completed]
