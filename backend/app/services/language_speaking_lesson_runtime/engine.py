...
async def _resolve_package_id(
    ...
    *,
    ...
    index = elp_index_from_payload(row.promotion_readiness_json if row else None)
...
async def open_lesson_runtime(
    ...
    *,
    ...
    pid = await _resolve_package_id(
        db, student_id=student_id, language_id=language_id, package_id=package_id, row=row
    )
