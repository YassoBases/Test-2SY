  ...
  const msg =
    typeof detail === 'string'
      ...
      : typeof detail?.message === 'string'
        ? detail.message
        : err?.message || ''
  return /No Learning Package available to open/i.test(msg) || /no_package/i.test(String(detail || ''))
}
...
async function onStartJourneySession() {
  ...
  try {
    ...
    const started = await startLearning({ authorMode: 'auto', useCache: true })
    if (started?.lesson) {
      hydrateLesson(started.lesson)
    } else {
      await openLesson({ packageId: started?.package?.package_id || ensuredPackageId.value })
    }
    try {
      await loadJourney({ force: true })
    } catch {
...
async function onContinueLearning() {
  ...
  try {
    ...
    try {
      await openLesson({ packageId: knownPackageId || undefined })
      enterLessonTab()
      return
    } catch (openErr) {
...
async function onLessonRestart() {
  try {
    await openLesson({ forceRestart: true })
  } catch {
    /* error on composable */
  }
}
...
async function onDiscussionOpen() {
  const opened = await ensureDiscussionSession({ forceOpen: true })
  if (!opened && !discussionState.value && !lessonReadyForDiscussion.value) {
    enterLessonTab()
  }
}
