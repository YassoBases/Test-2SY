  ...
  <div class="page-container slide-up-enter-active">
    ...
    <v-card v-else-if="!lesson" class="glass-card pa-6" variant="flat">
      <div class="text-center mb-5">
        ...
        <h3 class="text-h6 font-weight-bold mb-1">Personalized reading path</h3>
        <p class="text-body-2 text-medium-emphasis mb-4">
          Start with the passage that matches your level, next focus, and checkpoint goal.
        </p>
        <div v-if="introReadingPath" class="reading-path reading-path--intro pa-3 rounded-lg mb-4 text-start mx-auto" dir="ltr">
          <div class="d-flex align-center justify-space-between flex-wrap gap-2 mb-3">
            <div>
              <div class="text-caption font-weight-bold text-medium-emphasis">Your reading path</div>
              <div class="text-body-2 font-weight-bold">Next focus: {{ introReadingPath.next_focus?.label }}</div>
            </div>
            <div class="d-flex flex-wrap gap-1">
              <v-chip size="small" color="secondary" variant="tonal">Level {{ introReadingPath.level }}</v-chip>
              <v-chip size="small" color="info" variant="tonal">{{ introReadingPath.source_label }}</v-chip>
            </div>
          </div>

          <div class="intro-path-grid">
            <div class="intro-path-main">
              <div class="text-caption text-medium-emphasis mb-1">Strategy for the next passage</div>
              <div class="text-body-2 font-weight-medium">{{ introReadingPath.recommended_strategy }}</div>
              <div v-if="introReadingPath.next_focus?.hint" class="text-caption text-medium-emphasis mt-1">
                {{ introReadingPath.next_focus.hint }}
              </div>
            </div>

            <div>
              <div class="d-flex align-center justify-space-between text-caption mb-1">
                <span>Checkpoint to {{ introReadingPath.checkpoint?.next_level || 'next level' }}</span>
                <strong>{{ introReadingPath.checkpoint?.progress_percent || 0 }}%</strong>
              </div>
              <v-progress-linear
                :model-value="introReadingPath.checkpoint?.progress_percent || 0"
                height="8"
                rounded
                color="secondary"
              />
              <div class="text-caption text-medium-emphasis mt-1">
                {{ introReadingPath.checkpoint?.lessons_completed || 0 }}/{{ introReadingPath.checkpoint?.required_passages || 4 }}
                strong passages
              </div>
            </div>

            <div>
              <div v-for="s in introReadingPath.skill_breakdown?.slice(0, 3)" :key="s.code" class="skill-line">
                <div class="d-flex justify-space-between text-caption mb-1">
                  <span>{{ s.label }}</span>
                  <strong>{{ s.mastery_percent }}%</strong>
                </div>
                <v-progress-linear :model-value="s.mastery_percent" height="7" rounded color="secondary" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="d-flex align-center justify-center gap-1 mb-4">
          <span class="text-caption text-medium-emphasis me-1">Passage length:</span>
          <v-chip
      ...
      <v-row>
        ...
        <v-col cols="12" md="7">
          <v-card class="glass-card pa-5 passage-card" variant="flat">
            ...
            <div v-if="lesson.target_focus || lesson.mini_lesson" class="focus-box pa-3 rounded-lg mb-3" dir="ltr">
              <div class="text-caption font-weight-bold text-medium-emphasis mb-1">
                Focus: {{ lesson.target_focus || 'Reading strategy' }}
              </div>
              <div v-if="lesson.practice_hint" class="text-body-2 mb-2">{{ lesson.practice_hint }}</div>
              <div v-if="lesson.mini_lesson?.steps?.length" class="d-flex flex-wrap gap-1">
                <v-chip v-for="step in lesson.mini_lesson.steps" :key="step" size="x-small" variant="tonal" color="secondary">
                  {{ step }}
                </v-chip>
              </div>
            </div>
        ...
        <v-col cols="12" md="5">
          <v-card class="glass-card pa-5" variant="flat">
            ...
            <template v-else>
              ...
              <div v-if="result.component_breakdown?.length" class="component-box pa-3 rounded-lg mb-3" dir="ltr">
                <div class="text-caption font-weight-bold mb-2">Reading breakdown</div>
                <div v-for="c in result.component_breakdown" :key="c.code" class="skill-line">
                  <div class="d-flex justify-space-between text-caption mb-1">
                    <span>{{ c.label }}</span>
                    <strong>{{ Math.round(c.score_percent) }}%</strong>
                  </div>
                  <v-progress-linear :model-value="c.score_percent" height="7" rounded :color="c.score_percent >= 70 ? 'success' : 'warning'" />
                </div>
                <div v-if="result.next_focus?.hint" class="text-caption text-medium-emphasis mt-2">
                  Next focus: {{ result.next_focus.label }} - {{ result.next_focus.hint }}
                </div>
              </div>
...
const readingProfile = computed(() => insights.value?.reading_profile || null)
const CEFR = ['A1', 'A2', 'B1', 'B2', 'C1', 'C2']

const FALLBACK_READING_FOCUS = {
  A1: {
    label: 'Finding clear details',
    hint: 'Look for names, places, times, and simple actions before answering.',
    strategy: 'Read once for the story, then scan again for who, where, and what happened.',
    skills: [
      ['reading.scan_detail', 'Finding details', 45],
      ['reading.main_idea', 'Main idea', 40],
      ['reading.vocab_context', 'Word meaning', 38],
    ],
  },
  A2: {
    label: 'Main idea + details',
    hint: 'Choose the answer that matches the whole paragraph, not only one familiar word.',
    strategy: 'Read the first and last sentence, then scan the passage for proof.',
    skills: [
      ['reading.main_idea', 'Main idea', 48],
      ['reading.scan_detail', 'Finding details', 46],
      ['reading.sequence', 'Sequence', 42],
    ],
  },
  B1: {
    label: 'Inference',
    hint: 'Use clues in the text to answer what the writer means but does not say directly.',
    strategy: 'Mark the sentence that proves your answer before choosing an option.',
    skills: [
      ['reading.inference', 'Inference', 52],
      ['reading.main_idea', 'Main idea', 56],
      ['reading.vocab_context', 'Word meaning', 50],
    ],
  },
  B2: {
    label: 'Author purpose',
    hint: 'Decide why the writer includes a detail: example, contrast, reason, or opinion.',
    strategy: 'Separate facts from opinions, then connect each paragraph to the main claim.',
    skills: [
      ['reading.author_purpose', 'Author purpose', 58],
      ['reading.inference', 'Inference', 56],
      ['reading.vocab_context', 'Academic vocabulary', 54],
    ],
  },
  C1: {
    label: 'Tone and argument',
    hint: 'Track how the writer builds the argument and where the tone changes.',
    strategy: 'Summarize each paragraph in one phrase before answering detailed questions.',
    skills: [
      ['reading.author_purpose', 'Author purpose', 64],
      ['reading.inference', 'Inference', 62],
      ['reading.synthesis', 'Synthesis', 58],
    ],
  },
  C2: {
    label: 'Synthesis',
    hint: 'Compare claims, assumptions, and evidence across the whole passage.',
    strategy: 'Identify the writer stance, the strongest evidence, and any hidden assumption.',
    skills: [
      ['reading.synthesis', 'Synthesis', 70],
      ['reading.author_purpose', 'Author purpose', 68],
      ['reading.inference', 'Inference', 66],
    ],
  },
}
...
function normalizeCefr(level) {
  ...
  return CEFR.includes(clean) ? clean : 'A2'
}

function nextCefr(level) {
  const idx = CEFR.indexOf(normalizeCefr(level))
  return CEFR[Math.min(idx + 1, CEFR.length - 1)]
}

function storedReadingLevel() {
  try {
    return localStorage.getItem('reading_seen_level') || ''
  } catch {
    return ''
  }
}

function fallbackReadingPath(level) {
  const normalized = normalizeCefr(level)
  const focus = FALLBACK_READING_FOCUS[normalized] || FALLBACK_READING_FOCUS.A2
  const completed = Number(insights.value?.lessons_completed || 0)
  const avg = Number(insights.value?.avg_comprehension || 0)
  const progress = Math.min(100, Math.round(((Math.min(completed, 4) / 4) * 65) + (avg >= 80 ? 35 : 0)))
  return {
    level: normalized,
    source_label: completed ? 'Personalizing' : 'Starter path',
    next_focus: { label: focus.label, hint: focus.hint },
    recommended_strategy: focus.strategy,
    skill_breakdown: focus.skills.map(([code, label, mastery]) => ({
      code,
      label,
      mastery_percent: mastery,
    })),
    checkpoint: {
      next_level: nextCefr(normalized),
...
const introReadingPath = computed(() => {
  const profile = readingProfile.value
  if (profile?.next_focus) {
    const fallback = fallbackReadingPath(profile.level)
    return {
      ...profile,
      source_label: 'Personalized',
      recommended_strategy: profile.recommended_strategy || profile.next_focus?.hint || 'Read, find proof in the text, then answer.',
      skill_breakdown: profile.skill_breakdown?.length ? profile.skill_breakdown : fallback.skill_breakdown,
      checkpoint: profile.checkpoint || fallback.checkpoint,
    }
  }
  const fromHistory = history.value?.find((h) => h.level)?.level
  return fallbackReadingPath(fromHistory || storedReadingLevel() || 'A2')
})
