...
export function useGrammarEngineHome() {
  ...
  function canOpenStage(stage) {
    ...
    if (stage.status === 'current' || stage.status === 'completed') return true
    const stageRank = CEFR_RANK[String(stage.cefr || '').toUpperCase()] || 0
    const anchorRank = CEFR_RANK[String(anchorCefr.value || '').toUpperCase()] || 0
    return stage.status === 'unlocked' && stageRank > 0 && anchorRank > 0 && stageRank < anchorRank
  }
  ...
  async function startCurrentLesson({ grammarId = null, navigate = true } = {}) {
    ...
    // Locked / future stages must not start; completed/current/lower-band review may open.
    if (stage && !canOpenStage(stage)) {
      errorMessage.value = 'lesson'
  ...
  async function finishLesson(completionPayload = {}) {
    ...
    try {
      const result = await completeGrammarActivity({
        ...
        language_id: 1,
        answers: completionPayload?.answers || {},
        response_text: '',
      })
