    ...
    <template v-else>
      ...
      <section v-if="practiceTasks.length" class="lesson-part lesson-part--interactive" aria-labelledby="part-practice">
        ...
        <div v-else class="practice-summary" dir="rtl" lang="ar">
          ...
          <p>صحيح من أول محاولة: {{ firstTryCorrectCount }} من {{ practiceTasks.length }}.</p>
          <p>احتاج مساعدة: {{ helpedPracticeCount }}.</p>
          <p>تابع الآن واستخدم الجمل عن نفسك بهدوء.</p>
          <button type="button" class="primary-button" @click="focusProductionSection">
            تابع إلى استخدمها بنفسك
          </button>
        </div>
      </section>

      <section v-if="useTasks.length || hasReflection" id="grammar-production-section" class="lesson-part lesson-part--interactive" aria-labelledby="part-use">
        <p class="part-number">5</p>
        <h2 id="part-use" dir="rtl" lang="ar">استخدمها بنفسك</h2>

        <div class="production-card" dir="rtl" lang="ar">
          <p class="production-title">{{ compactProduction.prompt }}</p>
          <div class="starter-lines">
            <label v-for="line in compactProduction.starters" :key="line" class="starter-line">
              <span dir="ltr" lang="en">{{ line }}</span>
              <input type="text" aria-label="Personal sentence" dir="ltr" lang="en" />
            </label>
          </div>
        </div>

        <div v-if="showReflection" class="reflection-panel">
          <p v-if="content.reflection?.summary" :dir="textDir(content.reflection.summary)">
            {{ shortText(content.reflection.summary, 160) }}
          </p>
          <p
            v-if="content.reflection?.next_step || content.reflection?.next_focus"
            class="soft-copy"
            :dir="textDir(content.reflection.next_step || content.reflection.next_focus)"
          >
            {{ shortText(content.reflection.next_step || content.reflection.next_focus, 160) }}
          </p>
        </div>
      </section>
    </template>
...
import { evaluateGrammarLessonPreviewPractice } from '../../api/grammar.js'
...
const practiceTasks = computed(() => orderedPracticeTasks())
...
const compactProduction = computed(() => ({
  prompt: 'اكتب ثلاث جمل قصيرة: من أنت؟ أين أنت؟ كيف تشعر اليوم؟',
...
async function checkPracticeAnswer() {
  ...
  try {
    const result = await evaluateGrammarLessonPreviewPractice({
...
function focusProductionSection() {
  document.getElementById('grammar-production-section')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
}
  ...
  setup(taskProps, { emit }) {
    ...
    function renderInput(task, id) {
      ...
      if (task.type === 'reorder' || task.type === 'sentence_builder') {
        ...
        return h('div', { class: 'reorder-block' }, [
          ...
          h(
            'div',
            { class: 'reorder-answer', dir: 'ltr', lang: 'en' },
            selected.length
      ...
      if (task.type === 'open_response') {
        return h('div', { class: 'open-response-block' }, [
          ...
          h('textarea', {
            ...
            lang: 'en',
            value: taskProps.responses[task.id] || '',
            placeholder: task.scaffold || 'I am ...',
      ...
      return h('textarea', {
        ...
        lang: 'en',
        value: taskProps.responses[task.id] || '',
        placeholder: task.scaffold || 'Write here.',
        'aria-label': task.prompt || 'Write your response',
        disabled: taskProps.disabled,
        onInput: (event) => update(event.target.value),
      })
    ...
    return () => {
      ...
      return h('div', { class: 'task-panel' }, [
        ...
        task.scaffold
          ? h('p', { class: 'task-scaffold', dir: textDir(task.scaffold) }, task.scaffold)
          : null,
        renderInput(task, id),
