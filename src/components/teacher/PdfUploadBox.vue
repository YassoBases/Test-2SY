<template>
  ...
  >
    ...
    <div v-if="error && !uploading" class="upload-state">
      ...
      <h3 class="section-title text-h5 mb-2 text-error">{{ $t('teacher.pdf.uploadFailed') }}</h3>
      ...
      <v-btn class="btn-glow-outline" prepend-icon="mdi-folder-open" @click.stop="triggerFileInput">
        {{ $t('teacher.actions.chooseAnotherFile') }}
      </v-btn>
    </div>
    ...
    <div v-else-if="uploading" class="upload-state">
      ...
      <h3 class="section-title text-h5 mb-2">{{ $t('teacher.status.uploadingFile') }}</h3>
      ...
      <div class="progress-wrap mx-auto">
        ...
        <div class="d-flex justify-space-between mt-2 text-caption">
          <span class="text-medium-emphasis">{{ $t('teacher.labels.progress') }}</span>
    ...
    <template v-else>
      <div class="upload-state upload-state--success">
        ...
        <v-chip color="success" variant="tonal" prepend-icon="mdi-check" class="mb-4">
          {{ $t('teacher.status.uploadSuccess') }}
        </v-chip>
...
const ariaLabel = computed(() => {
  if (props.error) return t('teacher.pdf.uploadErrorRetry')
  if (props.uploading) return t('teacher.status.uploadingFileProgress')
