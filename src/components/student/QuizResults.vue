  ...
  >
    <div class="text-center mb-6">
      ...
      <p class="text-body-2 text-medium-emphasis">
        {{ t('student.lesson.quiz.results.summary', { correct: correctCount, total }) }}
      </p>
    </div>
    ...
    <v-row dense class="mb-4">
      <v-col cols="6">
        <div class="result-stat eduspark-gradient-soft pa-3 rounded-xl text-center">
          ...
          <div class="text-caption">{{ t('student.lesson.quiz.results.correct') }}</div>
        </div>
      </v-col>
      <v-col cols="6">
        <div class="result-stat eduspark-gradient-soft pa-3 rounded-xl text-center">
          ...
          <div class="text-caption">{{ t('student.lesson.quiz.results.wrong') }}</div>
        </div>
      </v-col>
    </v-row>
    ...
    <v-btn
      ...
      @click="$emit('retry')"
    >
      {{ t('student.lesson.quiz.results.retry') }}
    </v-btn>
    <v-btn
      v-else
      block
      ...
      :to="dashboardTo"
    >
      {{ t('student.lesson.quiz.results.back') }}
    </v-btn>
  </v-card>
</template>
...
const dashboardTo = ROUTES.STUDENT_COURSES
...
const resultTitle = computed(() => {
  if (scorePercent.value >= 80) return t('student.lesson.quiz.results.excellentEmoji')
  if (scorePercent.value >= 50) return t('student.lesson.quiz.results.goodPractice')
  return t('student.lesson.quiz.results.review')
})
</script>
