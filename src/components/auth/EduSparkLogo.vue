<template>
  <div
    class="eduspark-logo"
    :class="[sizeClass, { 'eduspark-logo--landing': isLanding }]"
  >
    <div v-if="!isLanding" class="eduspark-logo__ring" aria-hidden="true" />
    <div :class="isLanding ? 'eduspark-logo__aura' : 'eduspark-logo__glass'">
      <img
        :src="logoSrc"
...
const props = defineProps({
  size: {
    type: String,
    default: 'auth',
    validator: (v) => ['auth', 'sidebar', 'compact', 'hero'].includes(v),
  },
  variant: {
    type: String,
    default: 'default',
    validator: (v) => ['default', 'landing'].includes(v),
  },
...
const sizeClass = computed(() => {
  if (isLanding.value) return 'eduspark-logo--landing'
...
.eduspark-logo--hero .eduspark-logo__ring {
  inset: -4px;
}

.eduspark-logo--hero .eduspark-logo__glass {
  border-width: 1px;
  box-shadow:
    0 0 20px rgba(124, 108, 240, 0.35),
    0 0 36px rgba(34, 211, 238, 0.15);
}

.eduspark-logo--landing {
  width: 100%;
  height: 100%;
}
...
.eduspark-logo--landing .eduspark-logo__img {
  max-width: 100%;
  max-height: 100%;
  filter: drop-shadow(0 0 16px rgba(124, 108, 240, 0.35))
    drop-shadow(0 0 28px rgba(34, 211, 238, 0.2));
}

.eduspark-logo--landing:hover {
