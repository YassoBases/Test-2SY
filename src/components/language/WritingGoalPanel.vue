<template>
  <v-card class="goals-card pa-6 mb-4" variant="flat">
    <div class="d-flex align-center justify-space-between flex-wrap gap-2 mb-2">
      <div>
        <div class="text-overline text-medium-emphasis">{{ t('student.languages.writingJourney.goals.eyebrow') }}</div>
        <h3 class="text-h6 font-weight-bold mb-0">{{ t('student.languages.writingJourney.goals.title') }}</h3>
      </div>
      <v-fade-transition>
        <span v-if="goalSaved" class="text-success text-caption">{{ t('student.languages.writingJourney.goals.saved') }}</span>
      </v-fade-transition>
    </div>
    <p class="text-body-2 text-medium-emphasis mb-5">{{ t('student.languages.writingJourney.goals.subtitle') }}</p>
