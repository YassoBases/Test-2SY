<template>
  <div class="goals-panel glass-card mb-4">
    <div class="goals-panel__head">
      <div>
        <p class="goals-panel__eyebrow mb-0">{{ t('student.languages.listeningJourney.goals.eyebrow') }}</p>
        <h3 class="goals-panel__title">{{ t('student.languages.coach.ux.goals.title') }}</h3>
        <p class="goals-panel__sub mb-0">{{ t('student.languages.coach.ux.goals.subtitle') }}</p>
      </div>
      <v-fade-transition>
        <span v-if="goalSaved" class="goals-panel__saved">{{ t('student.languages.listeningJourney.goals.saved') }}</span>
      </v-fade-transition>
    </div>
...
.goal-tile {
  ...
  background:
    linear-gradient(180deg, rgba(var(--v-theme-surface), 0.92), rgba(var(--v-theme-surface), 0.72));
  color: inherit;
  cursor: pointer;
  transition:
...
.goal-tile--active {
  ...
  background:
    linear-gradient(165deg, rgba(var(--v-theme-secondary), 0.18), rgba(var(--v-theme-surface), 0.9) 55%);
  box-shadow:
